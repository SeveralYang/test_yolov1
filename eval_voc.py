# encoding:utf-8
#
# created by xiongzihua revised by yy
#
import os

os.environ["CUDA_VISIBLE_DEVICES"] = "0"

VOC_CLASSES = ('cat', 'tvmonitor', 'diningtable', 'car', 'bus', 'sofa', 'aeroplane',
               'bottle', 'train', 'chair', 'pottedplant', 'horse', 'cow', 'dog',
               'bicycle', 'boat', 'sheep', 'motorbike', 'bird', 'person')
...
if __name__ == '__main__':
    ...
    model = resnet50()
    model.load_state_dict(torch.load('yolo_49epoch.pth'))
    model.eval()
    model.cuda()
    count = 0
    for image_path in tqdm(image_list):
        result = predict_gpu(model, image_path, root_path='D:\ObjectDection\dataset\VOCdevkit\VOC2007\JPEGImages')
        ...
        image = cv2.imread(os.path.join('D:\ObjectDection\dataset\VOCdevkit\VOC2007\JPEGImages', image_path))
        ...
        cv2.imwrite(f"result_imgs/{image_path}", image)
